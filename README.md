...
-   **Auditing Fields**: Although not explicitly required by the exercise, all database tables include auditing fields (`CREATION_USER`, `CREATION_DATE`, `MODIFICATION_USER`, `MODIFICATION_DATE`). This is a forward-looking design choice and a best practice for data governance, providing a clear trace of when and by whom data was created or modified. The management of these fields is automated using a base `AuditableEntity` class with JPA lifecycle callbacks.
